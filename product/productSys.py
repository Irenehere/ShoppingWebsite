...
def add_product(js):
    ...
    sql = "insert into T_product (productid,productname," \
        ...
        pdinfo["productimage"],pdinfo["productdescribe"],pdinfo["producttype"]
    )
    stm = executeSql(sql)
...
def add_attribute(js):
    ...
    attrs = pdattr["attrs"]
    snum = searchDB('select count(storeid) from T_Store')[0][0]
    storeid = 's' + str(snum + 1).zfill(6)
...
def delete_attribute(js):
    ...
    如果成功删除返回“1”，否则返回“0”  '''
    pdattr = json.loads(js)
    storeid = searchDB("select storeid from T_productattribute "
                       "where productid = '{}' and attribute = '{}'".format(pdattr["productid"],
                                                                            pdattr["attrs"][0]["attrvalue"]))[0][0]
    sql = "delete from T_productattribute where productid = '{}' and storeid = '{}'".format(pdattr["productid"],
                                                                                           storeid)
    stm1 = executeSql(sql)
